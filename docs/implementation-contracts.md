...
The qualified driver is exactly `7.2.0`, release commit `cc7751c01c3d3956d061073283bee6b23bf33422`, pinned in the package and lockfile. It carries the network-guard fixes, independent report execution and the operation-settlement API below, and its 133 operations and 48 additional helpers match the versioned inventory. Its acceptance checks run as standing tests against the installed artifact; one deviation is recorded in [driver qualification](driver-qualification.md), where a rate-limited report generation is still retried. Update provenance and regression evidence with any future dependency change; do not guess a version number.

`run_report` and `run_cross_project_report` are GET endpoints with side effects. Their public driver methods must use `retry: 'none'` and `bypassCache: true`, which must bypass both result caching and in-flight GET coalescing. Require upstream tests proving separate sequential and concurrent calls execute separately and that network and 5xx failures are not retried. A rate-limited (429) response is exempt: TestRail rejects it before handling the request, so a re-send cannot generate the report twice or send a duplicate template-configured email. A 5xx is not exempt, because the server may already have begun generating. The MCP adapter continues to call the public report methods. This prerequisite can proceed alongside adapter development, but blocks report qualification and production release. Report execution can send template-configured email and returns a URL before the file is ready; describe those effects and never poll by executing the report again. Sources: [driver report methods](https://github.com/dichovsky/testrail-api-client/blob/89f636e276ea701412bb06039e3b963d83126ea1/src/modules/reports.ts), [TestRail report API](https://support.testrail.com/hc/en-us/articles/7077825062036-Reports-and-Cross-Project-Reports).

The inspected driver can reject a public method at an internal deadline while DNS, fetch or response-body cleanup is still pending. Observing only that method's promise, or a fetch wrapper that resolves at response headers, cannot prove resource settlement. Require a public per-operation handle from the driver; the proposed API contract is `client.trackOperation(() => client.projects.getAllProjects(...))` returning `{ result: Promise<T>, settled: Promise<void> }`. This API is exported by the qualified 7.2.0 driver and was not present in 7.0.0. `result` retains the normal method value/error/deadline semantics. `settled` resolves only after the callback has finished and every started or joined descendant has settled or been conclusively stopped: DNS, fetch attempts, body reads and cancellation completion, upload streams, retry timers, and shared/coalesced requests. A requested abort/cancel alone is not settlement. Late failures are observed and do not produce unhandled rejections; any later descendants remain owned by the same handle. The driver owns this accounting so the adapter does not copy its transport engine.

F01 tests must show that an aggregate result rejects at its deadline while `settled` remains pending for deferred DNS/fetch/body cancellation; releasing those tasks eventually settles the handle and late DNS must not start a new fetch after the deadline. F03 tests fill all four invocation slots with such handles, receive deadline errors, and prove subsequent calls are `BUSY` with no new DNS/fetch work until genuine settlement releases capacity. Include coalesced callers and multipart cleanup. The four-slot limit bounds owned invocation scopes; it is not a claim that each invocation uses only one network primitive. The driver still bounds retries and aggregate pages within a scope.
...
Reject new calls with `BUSY` when capacity is exhausted; no unbounded queue. Reserve the download slot before a binary request. Invoke each public method through the qualified driver's operation handle, use `result` for response/error and TestRail acknowledgment classification, and retain slots until both `settled` and adapter-owned file work finish. This applies even when an aggregate's own deadline rejects before the adapter watchdog. Neither the public method result nor a watchdog/`Promise.race` rejection may release capacity while descendants continue.
...
Keep one reviewed declarative entry per endpoint. Each entry includes the REST method/route/token, exact tool name, family, public driver binding, input schema, explicit argument map, required outer response shape, advisory entity schema, pagination capability, file behavior, TestRail mutation classification and MCP side-effect annotations. Keep TestRail mutation classification separate from local filesystem effects. Use the same registry to register tools and generate references and parity checks, but compare it with the independent pinned inventory and independent parameter fixtures.
...
Descriptions include effects, required identifiers, pagination limitations and file semantics as applicable, under 2 KiB each. Annotations describe the entire tool, including local effects: reads without persistent local changes are read-only/idempotent; mutations are read-only false, with conservative destructive/idempotent hints reviewed individually. Both report generators are read-only false and idempotent false despite GET. `testrail_get_attachment` uses `readOnlyHint: false`, `destructiveHint: false` and `idempotentHint: false`: each successful call creates a new persistent local file without overwriting existing files. Its TestRail request retains the ordinary GET retry policy; MCP effect hints do not select upstream retry policy. All operations interact with an external TestRail instance (`openWorldHint: true`). Hints do not grant permissions or add confirmation checks.
...
For every operation that mutates TestRail or initiates report generation, attach `write_outcome` to tool errors. Do not derive this classification solely from MCP `readOnlyHint`: attachment downloads have local file effects but do not mutate TestRail, so their errors do not receive a TestRail `write_outcome`.
