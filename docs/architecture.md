...
Pin `@dichovsky/testrail-api-client` to the qualified published release **7.2.0**. It contains the network-guard fixes, the fixes preventing report execution from being cached or coalesced, and the public per-operation result/settlement handle specified in the contracts; without that handle a public method deadline can reject before its DNS/fetch/body descendants settle. Its qualification checks run as standing tests against the installed artifact, with one recorded deviation for rate-limited report retries. Re-run parity and contract tests against any future exact version before release. Do not substitute a floating branch, an independent HTTP implementation, or silent omission of affected endpoints. See [implementation contracts](implementation-contracts.md) and [implementation plan](implementation-plan.md) for the release dependencies and verification.
