 ...
 * 403 as a licence restriction only when its message names an Enterprise licence or
 * subscription in the phrasing the driver recognises, so a refusal worded otherwise arrives as an
...
export const getCrossProjectReports = defineOperation({
  ...
  summary: 'List the cross-project report templates available to the API that the configured user can access, which testrail_run_cross_project_report runs by id. A template\'s notify_* fields record whether and to whom running it sends email. TestRail documents this endpoint as Enterprise only and answers 403 on an instance without Enterprise or when the user\'s role does not grant access. A 403 is reported as LICENSE_REQUIRED only when its message says "not an" or "requires" followed by "Enterprise licen" or "Enterprise subscription"; a 403 worded otherwise, TestRail\'s documented "TestRail Enterprise only. Access denied." among them, arrives as PERMISSION_DENIED, which here may still mean the instance lacks Enterprise.',
  inputSchema: noInput,
  argumentMap: [],
...
export const runCrossProjectReport = defineOperation({
  ...
  summary: 'Run a cross-project report template, one listed by testrail_get_cross_project_reports, and return the URLs TestRail gives for the new report: report_url, with report_html and report_pdf. TestRail documents this endpoint as Enterprise only. Each run TestRail executes generates a new report, and the template\'s notify settings may email it. TestRail\'s reference warns that a report may not be available immediately after being run, in the introduction that precedes all four report endpoints but opens by speaking of single-project reports, and the cross-project sections do not repeat the warning, so treat the URLs as possibly not ready. Neither this server nor its driver retries a run after a network error or a 5xx, since TestRail may already have begun it; the driver re-sends only a rate-limited (429) request, which TestRail rejects before handling. If a run fails after the request was sent, acknowledged means TestRail returned a success reply this server could not use, and unknown that this server cannot tell whether a report was generated; unknown is also what a refusal from TestRail carries, with its code and http_status saying what TestRail answered; a RATE_LIMITED 429 can instead come from the driver\'s own rate limiter, which refuses before sending and still reports unknown. Running the template again to check could generate and email a second report. TestRail answers 400 for an invalid report template ID and 403 when the configured user has no access to the project. A 403 is reported as LICENSE_REQUIRED only when its message says "not an" or "requires" followed by "Enterprise licen" or "Enterprise subscription"; a 403 worded otherwise, TestRail\'s documented "TestRail Enterprise only. Access denied." among them, arrives as PERMISSION_DENIED, which here may still mean the instance lacks Enterprise.',
